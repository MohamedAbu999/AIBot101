...
def perform_analysis_and_trade():
    global candles, selected_market, last_trade_time, current_balance, stake, rf_model, last_model_train_time, consecutive_losses, last_loss_time, is_profit_locked
    ...
    if len(df) >= 2:
        ...
        if not (prev_candle["close"] > prev_candle["open"] and current_candle["close"] > current_candle["open"]):
            if DEBUG_MODE:
...
def execute_trade():
    ...
    log_trade(profit, trade_outcome)
